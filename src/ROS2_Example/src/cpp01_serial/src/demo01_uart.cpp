...
public:
  Serial_Node() : Node("serial_node_cpp")
  {
    ...
    // 串口设备名（根据实际设备调整）
    const std::string device_name = "/dev/pts/5";
    ...
    Subscription_ = this->create_subscription<geometry_msgs::msg::Twist>("/cmd_vel", 10,
                                                                         std::bind(&Serial_Node::speed_recieve_callback, this, std::placeholders::_1));
...
private:
  ...
  void send_message_timer_callback()
  {
    ...
    num2 = this->liner_y;
    num3 = this->angular_z;
    // fp32 fp32_buffer[] = {num1,num2,num3};
    // fp32 fp32_buffer[] = {1.0,2.0,3.0};
    ...
    try
    {
      ...
      // RCLCPP_INFO(this->get_logger(),"发送的数据是:%.2f,%.2f,%.2f",1,2,3);
      RCLCPP_INFO(this->get_logger(), "发送的数据是:%.2f,%.2f,%.2f", num1, num2, num3);
  ...
  void async_receive_message() // 创建一个函数更方便重新调用
  {
    ...
    port->async_receive([this](const std::vector<uint8_t> &data, const size_t &size)
        ...
        if (size > 0)
        {
          ...
          if(serial_pack_.rx.data.cmd == 0x00)
          {
            for(auto i:serial_pack_.rx.data.fp32_buffer)
            {
              RCLCPP_INFO(this->get_logger(),"接收到的浮点形是:%.2f",i);
            }
