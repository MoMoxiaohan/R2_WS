...
public:
    Robot2Controller() : Node("robot2_controller") {
        // PID参数
        kp_ = 0.5;
        ki_ = 0.01;
        kd_ = 0.1;
        ...
        // 发布角速度控制命令
        omega_pub_ = this->create_publisher<std_msgs::msg::Float32>("/omega2", 10);
...
private:
    ...
    void calculate_control() {
        // 计算期望角度（指向机器人1）
        double desired_yaw = atan2(robot1_y_ - robot2_y_, robot1_x_ - robot2_x_);
        
        // 计算角度误差（归一化到[-π, π]）
        double error = desired_yaw - robot2_yaw_;
